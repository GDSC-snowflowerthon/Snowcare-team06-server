...
//        String accessToken = jwtTokenProvider.generate(subject, accessTokenExpiredAt, email, user.getRole());
//        String refreshToken = jwtTokenProvider.generate(subject, refreshTokenExpiredAt, email, user.getRole());
//
//        redisUtil.setDataExpire(String.valueOf(userId), refreshToken, refreshTokenExpiredAt.getTime()); // 시간은 long 타입으로
//
//        return AuthTokens.of(accessToken, refreshToken, userId, newUser);
//    }
//
...
//        User user = userRepository.findById(Long.valueOf(userId))
//                .orElseThrow(() -> new CustomException(ErrorCode.NOT_FOUND_USER));
//        String newAccessToken = jwtTokenProvider.generate(userId, accessTokenExpiredAt, user.getEmail(), user.getRole());
